...
getDataFromCache(StringAddress, L, Data, HopsNum, directMap, BitsNum):-
	HopsNum is 0,
	convertAddress(ItemAddress, BitsNum, Tag, Idx, directMap),
	convertBinToDec(Idx, DecimalIdx),
	nth0(DecimalIdx, L, item(tag(StringTag), data(Data), 1,_)),
	atom_number(StringTag, Tag),
	atom_number(StringAddress, ItemAddress).
